...
const formulas = [
  {
    ...
    formula: 'ΔT = Ts - Tp',
    description: 'Diferencia de tiempo entre la llegada de la onda S y la onda P.',
  ...
  },
  {
    ...
    description: 'Estimación de la distancia al epicentro usando las velocidades promedio de ondas P y S.',
    result: `D ≈ ${deltaSP.value} × (${vp} × ${vs.toFixed(2)}) / (${vp} - ${vs.toFixed(2)}) ≈ ${distanceEpicentral.value} km`,
    color: 'igp-orange',
  },
]
...
<template>
  <div class="space-y-8">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
      <h3 class="text-lg font-bold text-igp-dark-blue-500 mb-3">Cálculos Fundamentales</h3>
      <p class="text-gray-600 leading-relaxed">
        Una vez identificadas las llegadas de las ondas P y S, el siguiente paso es realizar
        cálculos que nos permitan obtener la distancia epicentral. Estos cálculos son la base
        para la localización del evento sísmico.
      </p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      >
        <h4 class="font-bold text-igp-dark-blue-500 mb-2 text-base">{{ f.title }}</h4>
        <p class="text-sm text-gray-500 mb-4">{{ f.description }}</p>

        <!-- Formula display -->
        <div class="bg-gray-50 rounded-xl p-4 mb-4 text-center">
          <p class="text-lg font-mono font-semibold text-igp-dark-blue-500">{{ f.formula }}</p>
        </div>
    ...
    <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
      <h3 class="text-lg font-bold text-igp-dark-blue-500 mb-2 flex items-center gap-2">
        <AppIcon name="calculator" :size="20" class="text-igp-sky-blue-600" />
        Ejercicio guiado
      </h3>
      <p class="text-sm text-gray-500 mb-6">Sigue estos pasos para calcular la distancia epicentral:</p>
      ...
      <div class="space-y-3">
        ...
        >
          <div class="w-8 h-8 rounded-lg bg-igp-dark-blue-500 text-white text-sm font-bold flex items-center justify-center flex-shrink-0">
            {{ index + 1 }}
          </div>
          <div class="flex-1">
            <p class="text-sm font-medium text-gray-700">{{ step.label }}</p>
          </div>
          <div class="bg-white px-3 py-1.5 rounded-lg border border-gray-200 text-sm font-mono font-semibold text-igp-dark-blue-500">
            {{ step.value }}
          </div>
        </div>
      </div>
    ...
    <div class="bg-igp-dark-blue-500 rounded-2xl p-6 text-white">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div>
          <label class="text-xs font-medium text-gray-300 mb-1 block">Tiempo Onda P (Tp) en segundos</label>
          <input
            v-model.number="tP"
            type="number"
            step="0.1"
        ...
        <div>
          <label class="text-xs font-medium text-gray-300 mb-1 block">Tiempo Onda S (Ts) en segundos</label>
          <input
            v-model.number="tS"
            type="number"
            step="0.1"
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div class="bg-white/10 rounded-xl p-4 text-center">
          ...
          <p class="text-2xl font-bold text-igp-sky-blue-400">{{ deltaSP }} s</p>
        </div>
        <div class="bg-white/10 rounded-xl p-4 text-center">
          <p class="text-xs text-gray-400 mb-1">Distancia Epicentral</p>
          <p class="text-2xl font-bold text-igp-sky-blue-400">{{ distanceEpicentral }} km</p>
        </div>
      </div>
    </div>
    ...
    <AlertBox icon="info" variant="success">
      ...
      <p>
        Estas fórmulas son simplificaciones educativas. En la práctica profesional,
        se utilizan tablas de viaje (travel-time tables) y modelos de velocidad más
        complejos para obtener distancias epicentrales precisas.
      </p>
    </AlertBox>
  </div>
</template>
